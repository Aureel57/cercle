...
function RangeCal({sd,setSd,ed,setEd,blockedDates}){
  ...
  const[viewM,setViewM]=useState(initM.getMonth());
  const[picking,setPicking]=useState("start");// "start" | "end"
  const[hovered,setHovered]=useState(null);
  const blockedSet=new Set(blockedDates||[]);
  const toKey=(y,m,d)=>`${y}-${String(m+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`;
  const startD=sd?new Date(sd):null;if(startD)startD.setHours(0,0,0,0);
  const endD=ed?new Date(ed):null;if(endD)endD.setHours(0,0,0,0);
  const hovD=hovered?new Date(hovered):null;if(hovD)hovD.setHours(0,0,0,0);
  ...
  const fmtDate=s=>{if(!s)return null;const[y,m,d]=s.split("-");return `${d}/${m}/${y.slice(2)}`;};

  const handleDay=(y,m,day)=>{
    const key=toKey(y,m,day);
    const dt=new Date(y,m,day);dt.setHours(0,0,0,0);
    if(dt<today||blockedSet.has(key))return;
    if(picking==="start"){
      setSd(key);
      if(endD&&dt>endD){setEd(key);}
      setPicking("end");
    } else {
      if(startD&&dt<startD){setSd(key);setEd(key);setPicking("end");}
      else{setEd(key);setPicking("start");}
    }
  };

  const daysInMonth=new Date(viewY,viewM+1,0).getDate();
  const firstDay=(new Date(viewY,viewM,1).getDay()||7)-1;// 0=Mon
  ...
  return <div>
    ...
    <div style={{background:"var(--w)",borderRadius:16,border:"1.5px solid var(--bd)",overflow:"hidden",boxShadow:"var(--sh)"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",alignItems:"center",padding:"8px 10px",gap:4,borderBottom:"1px solid var(--bd)"}}>
        <div onClick={()=>setPicking("start")} style={{padding:"6px 8px",borderRadius:9,border:"1.5px solid "+(picking==="start"?"var(--p)":"var(--bd)"),background:picking==="start"?"#f5f3ff":"var(--bgw)",cursor:"pointer",transition:"all .15s"}}>
          <div style={{fontSize:8,fontWeight:800,color:picking==="start"?"var(--p)":"var(--g)",textTransform:"uppercase",letterSpacing:".07em",marginBottom:1}}>Arrivée</div>
          <div style={{fontSize:12,fontWeight:700,color:"var(--dk)"}}>{sd?fmtDate(sd):<span style={{color:"var(--gl)"}}>—</span>}</div>
        </div>
        <span style={{fontSize:13,color:"var(--bd)"}}>→</span>
        <div onClick={()=>sd&&setPicking("end")} style={{padding:"6px 8px",borderRadius:9,border:"1.5px solid "+(picking==="end"?"var(--p)":"var(--bd)"),background:picking==="end"?"#f5f3ff":"var(--bgw)",cursor:sd?"pointer":"default",transition:"all .15s"}}>
          <div style={{fontSize:8,fontWeight:800,color:picking==="end"?"var(--p)":"var(--g)",textTransform:"uppercase",letterSpacing:".07em",marginBottom:1}}>Départ</div>
          <div style={{fontSize:12,fontWeight:700,color:"var(--dk)"}}>{ed?fmtDate(ed):<span style={{color:"var(--gl)"}}>—</span>}</div>
        </div>
      </div>

      {/* Navigation mois */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 12px",background:"var(--bgw)"}}>
        <button onClick={prev} style={{background:"var(--w)",border:"1.5px solid var(--bd)",borderRadius:7,width:26,height:26,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--dk)"}}>‹</button>
        <span style={{fontFamily:"var(--fd)",fontSize:13,fontWeight:700,color:"var(--dk)"}}>{MONTHS_FR[viewM]} {viewY}</span>
        <button onClick={next} style={{background:"var(--w)",border:"1.5px solid var(--bd)",borderRadius:7,width:26,height:26,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--dk)"}}>›</button>
      </div>

      {/* Grille jours */}
      <div style={{padding:"4px 8px 8px"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",marginBottom:1}}>
          {DAYS_FR.map(d=><div key={d} style={{textAlign:"center",fontSize:9,fontWeight:700,color:"var(--gl)",padding:"2px 0"}}>{d}</div>)}
        </div>
      ...
      {/* Légende compacte */}
      <div style={{padding:"4px 10px 6px",borderTop:"1px solid var(--bd)",display:"flex",gap:8}}>
        <span style={{display:"flex",alignItems:"center",gap:3,fontSize:9,color:"var(--g)"}}><span style={{width:8,height:8,borderRadius:2,background:"var(--p)",display:"inline-block"}}/> Sél.</span>
        <span style={{display:"flex",alignItems:"center",gap:3,fontSize:9,color:"var(--g)"}}><span style={{width:8,height:8,borderRadius:2,background:"rgba(124,58,237,.1)",display:"inline-block"}}/> Période</span>
        {blockedDates?.length>0&&<span style={{display:"flex",alignItems:"center",gap:3,fontSize:9,color:"var(--g)"}}><span style={{width:8,height:8,borderRadius:2,background:"#fee2e2",display:"inline-block"}}/> Indispo</span>}
      </div>
    </div>

    {/* Hint */}
    <div style={{textAlign:"center",fontSize:10,color:"var(--p)",fontWeight:600,marginTop:4,opacity:.8}}>
      {picking==="start"?"📅 Date d'arrivée ?":"📅 Date de départ ?"}
    </div>
  </div>;
}
