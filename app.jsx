...
function reducer(s,a){switch(a.type){
  ...
  case"ADD_ITEM":{const it={...a.payload,id:a.payload.id||uid(),owner:a.payload.owner||s.user,rating:0,reviews:0,createdAt:a.payload.createdAt||new Date().getFullYear().toString(),isPro:a.payload.isPro||false,_demo:false};const newUI=[...s.userItems,it];try{localStorage.setItem('cercle_items',JSON.stringify(newUI))}catch{}return{...s,userItems:newUI,notifications:[{id:uid(),text:`"${it.title}" publiée !`,read:false,at:new Date()},...s.notifications]}}
  case"SET_ITEMS":return{...s,userItems:[...a.items]};
...
function CreateListing({state,dispatch,setPage,mode}){
  ...
  const u=(k,v)=>setF(p=>({...p,[k]:v}));
  const go=()=>{if(!f.title||!f.price)return;const nid=uid();const payload={...f,price:+f.price,deposit:+f.deposit||+f.price*3,images:[mkImg(f.cat,99,0),mkImg(f.cat,99,1),mkImg(f.cat,99,2)],id:nid,owner:state.user,rating:0,reviews:0,createdAt:new Date().getFullYear().toString(),isPro:!!state.user?.isPro,_demo:false};if(window.db)window.db.collection('items').doc(nid).set(payload).catch(console.error);dispatch({type:"ADD_ITEM",payload});setPage("profile")};
...
function App(){
  ...
  // Load saved user + items on mount
  useEffect(()=>{
    try{const raw=localStorage.getItem('cercle_items');if(raw){const saved=JSON.parse(raw);if(saved&&saved.length)dispatch({type:'SET_ITEMS',items:saved})}}catch{}
    try{const s=window.storage;if(s){s.get('cercle_user').then(r=>{if(r&&r.value){const u=JSON.parse(r.value);dispatch({type:'LOGIN',payload:u})}}).catch(()=>{}).finally(()=>setLoaded(true))}else setLoaded(true)}catch(e){setLoaded(true)}
    if(window.db){const unsub=window.db.collection('items').onSnapshot(snap=>{const items=snap.docs.map(d=>({id:d.id,...d.data()})).filter(i=>!i._demo);if(items.length)dispatch({type:'SET_ITEMS',items})},()=>{});return unsub}
  },[]);
